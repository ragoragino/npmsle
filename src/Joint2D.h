...
struct JointParameters2D
{
	JointParameters2D(double gamma_p, double mu_p, double gamma_v, double mu_v, double beta_v, double sigma_v,
...
class WrapperSimulatedJoint2D
{
public:
	WrapperSimulatedJoint2D(double * price, double * volatility, double * sentiment, double dt, int N_obs, int N_sim,
...
template<typename GeneratorType = std::mt19937_64, typename GeneratorSeed = RandomStart>
void simulate_joint2D_process(double * price, double * volatility, double * sentiment, JointParameters2D *
...
template<typename GeneratorType = std::mt19937_64, typename GeneratorSeed = RandomStart>
double simulated_ll_joint2D(const std::vector<double>& x, std::vector<double>& grad, void* data)
{
	// Unwraping parameter
	double gamma_p = x[0];
	double mu_p = x[1];
